...
namespace Microsoft.Samples.CustomControls
{
    ...
    public class SpectrumSlider : Slider
    {
        ...
        ColorThumb m_Thumb;
        RepeatButton m_IncreaseRepeatButton;
        RepeatButton m_DecreaseRepeatButton;
        ...
        bool m_ControlingMouse = false;
        protected override void OnPreviewMouseDown(MouseButtonEventArgs e)
        {
            Point p = e.GetPosition(m_spectrumDisplay);
            if (m_spectrumDisplay.ActualHeight == 0)
                return;

            double currPos = ((1 - (p.Y / (m_spectrumDisplay.ActualHeight))) * (Maximum - Minimum)) + Minimum;
            ...
            Value = currPos;
            e.Handled = true;
            m_ControlingMouse = true;
        }

        protected override void OnMouseMove(MouseEventArgs e)
        {
            e.Handled = true;
            if (m_ControlingMouse)
            {
                Point p = e.GetPosition(m_spectrumDisplay);
                if (m_spectrumDisplay.ActualHeight == 0)
                    return;
        ...
        protected override void  OnPreviewMouseUp(MouseButtonEventArgs e)
        {
            ...
            m_ControlingMouse = false;
            e.Handled = true;
        }

        protected override void OnValueChanged(double oldValue, double newValue)
        {
            base.OnValueChanged(oldValue, newValue);  /* 360 - */
